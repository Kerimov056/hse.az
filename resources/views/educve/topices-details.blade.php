    ...
    <style>
    /* Card polish */
    .topic-hero{border-radius:14px; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,.08)}
    .topic-hero img{width:100%; height:440px; object-fit:cover}

    .badge-views{display:inline-flex; align-items:center; gap:.4rem; padding:.35rem .6rem;
    ...
    <section class="td_page_heading td_center td_bg_filed td_heading_bg text-center td_hobble"
        ...
        <div class="td_page_heading_in">
            <h1 class="td_white_color td_fs_48 td_mb_10">{{ $topic->name }}</h1>
            <ol class="breadcrumb m-0 td_fs_20 td_opacity_8 td_semibold td_white_color">
            <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
            <li class="breadcrumb-item"><a href="{{ route('topices') }}">Topics</a></li>
            <li class="breadcrumb-item active">{{ Str::limit($topic->name, 40) }}</li>
            </ol>
        </div>
        </div>
    </section>
    ...
    <section>
        ...
        <div class="row td_gap_y_30">
            {{-- MAIN --}}
            <div class="col-lg-8">
            <div class="topic-hero td_white_bg td_radius_10 td_mb_20">
                @if($topic->imageUrl)
                ...
                <div class="d-flex align-items-center justify-content-between td_mb_10">
                    ...
                    @if($topic->courseUrl)
                    <a href="{{ $topic->courseUrl }}" target="_blank" rel="noopener" class="td_btn td_style_2 td_radius_10 td_medium">
                        <span class="td_btn_in td_heading_color td_white_bg"><span>Visit</span></span>
                    </a>
                    @endif
                </div>

                <div class="topic-content td_content td_fs_18 td_heading_color td_opacity_9">
                    {!! $topic->description !!}
                </div>
                </div>
            </div>
            ...
            <div class="side-card td_white_bg td_radius_10">
                <div class="side-in">
                <h3 class="td_fs_24 td_mb_16">About this topic</h3>
                <ul class="td_mp_0 td_fs_18 td_heading_color td_mb_0">
                    <li class="td_mb_10"><b>Category:</b> {{ $topic->category->name ?? ($topic->category ?? 'Topic') }}</li>
                    <li><b>Updated:</b> {{ optional($topic->updated_at)->format('d.m.Y H:i') }}</li>
                </ul>
                </div>
            </div>

            {{-- Social links (RIGHT SIDEBAR — icon buttons with animations) --}}
        ...
        {{-- Related --}}
        @if($relatedTopices->count())
            <div class="td_height_60 td_height_lg_40"></div>
            <h2 class="td_fs_36 td_mb_30">Related Topics</h2>
            <div class="row td_gap_y_30 td_row_gap_30">
            @foreach($relatedTopices as $rc)
                <div class="col-lg-4 col-md-6">
                <div class="td_card td_style_3 td_radius_10">
                    <a href="{{ route('topices-details', $rc->id) }}" class="td_card_thumb">
                    ...
                    <div class="td_card_info_in">
                        <a href="{{ route('topices') }}" class="td_card_category td_fs_14 td_bold td_heading_color td_mb_10">
                        <span>{{ $rc->category->name ?? ($rc->category ?? 'Topic') }}</span>
                        </a>
                        <h3 class="td_card_title td_fs_22 td_mb_10">
                        <a href="{{ route('topices-details', $rc->id) }}">{{ $rc->name }}</a>
                        </h3>
                        ...
                        <a href="{{ route('topices-details', $rc->id) }}" class="td_btn td_style_1 td_radius_10 td_medium">
