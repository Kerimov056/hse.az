...
@section('content')
    ...
    @endif
    @if (!empty($q))
        <div class="alert alert-info py-2">Axtarış: <strong>{{ $q }}</strong> — <span
                class="text-muted">{{ $vacancies->total() }} nəticə</span></div>
    @endif
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-start gap-3">
                                    ...
                                    @endif
                                    <div class="min-w-0">
                                        <a href="{{ route('admin.vacancies.show', $v) }}"
                                            class="fw-semibold text-decoration-none d-block line-1">{{ $v->name }}</a>
                                        @if (!empty($v->description))
                                            <div class="small text-muted line-2">{{ strip_tags($v->description) }}</div>
                                        @endif
                                        @if ($v->courseUrl)
                                            <a href="{{ $v->courseUrl }}" target="_blank" rel="noopener"
                                                class="small text-decoration-none"><i class="bi bi-box-arrow-up-right"></i>
                                                aç</a>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <td class="d-none d-md-table-cell text-center">
                                @if ($v->imageUrl)
                                    <img src="{{ $v->imageUrl }}" alt="" class="thumb-lg">
                                @endif
                            </td>
                            <td class="text-end">
                                <div class="actions-wide btn-group" role="group">
                                    <a class="btn btn-sm btn-outline-primary"
                                        href="{{ route('vacancies-details', ['vacancy' => $v->getRouteKey()]) }}"
                                        target="_blank">
                                        <i class="bi bi-eye"></i> Gör
                                    </a>
                                    <a class="btn btn-sm btn-secondary" href="{{ route('admin.vacancies.edit', $v) }}"><i
                                            class="bi bi-pencil"></i> Edit</a>
                                    <form action="{{ route('admin.vacancies.destroy', $v) }}" method="post"
                                ...
                                <div class="actions-compact dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li><a class="dropdown-item" href="{{ route('vacancies-details', $v) }}"
                                                target="_blank"><i class="bi bi-eye me-2"></i>Gör</a></li>
                                        <li><a class="dropdown-item" href="{{ route('admin.vacancies.edit', $v) }}"><i
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted py-4">
                                {{ !empty($q) ? "“{$q}” üzrə heç nə tapılmadı." : 'Hələ vakansiya yoxdur.' }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
