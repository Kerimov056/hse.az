...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center page-actions mb-3">
  <h1 class="mb-0 fw-semibold">News</h1>

  <form action="{{ route('admin.news.index') }}" method="GET" class="flex-grow-1" style="max-width:540px">
    <div class="input-group">
      <div class="search-wrap w-100">
        <i class="bi bi-search"></i>
        <input type="search" name="q" value="{{ $q ?? '' }}" class="form-control"
               placeholder="Başlığa və ya mətnə görə axtar…" aria-label="Search news">
      </div>
      @if(!empty($q))
        <a href="{{ route('admin.news.index') }}" class="btn btn-outline-secondary">Təmizlə</a>
      @endif
  ...
  <a class="btn btn-primary" href="{{ route('admin.news.create') }}">
    <i class="bi bi-plus-lg me-1"></i> Yeni Xəbər
  </a>
</div>
...
@if(!empty($q))
  <div class="alert alert-info py-2">
    Axtarış: <strong>{{ $q }}</strong> —
    <span class="text-muted">{{ $news->total() }} nəticə</span>
  </div>
@endif
          ...
          <tr>
            ...
            <td>
              <div class="d-flex align-items-start gap-3">
                @if($n->imageUrl)
                  <img src="{{ $n->imageUrl }}" alt="" class="thumb d-none d-sm-block">
                @endif
                <div class="min-w-0">
                  <a href="{{ route('admin.news.show',$n) }}"
                     class="fw-semibold text-decoration-none d-block line-1">
                    {{ $n->name }}
                  </a>
                  ...
                  @endif
                  @if($n->courseUrl)
                    <a href="{{ $n->courseUrl }}" target="_blank" rel="noopener"
            ...
            <td class="d-none d-md-table-cell text-center">
              @if($n->imageUrl)
                <img src="{{ $n->imageUrl }}" alt="" class="thumb-lg">
              @endif
            </td>
            <td class="text-end">
              <div class="actions-wide btn-group" role="group">
                <a class="btn btn-sm btn-outline-primary"
                   href="{{ route('admin.news.show',$n) }}">
